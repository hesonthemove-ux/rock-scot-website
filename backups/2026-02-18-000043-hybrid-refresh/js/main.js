...
/* WIRE_MAIN_FALLBACK_V1 */
(function () {
  if (window.__WIRE_MAIN_FALLBACK_V1__) return;
  window.__WIRE_MAIN_FALLBACK_V1__ = true;
  ...
  var API = SUPABASE_URL + "/rest/v1/wire_news?select=title,created_at&is_live=eq.true&order=created_at.desc&limit=20";

  function targets() {
    var out = [];
    [
      "#wireTicker", "#wireTickerText", "#wire-news-ticker", "#wireNewsTicker", "#newsTicker",
      ".wire-ticker", ".wire-ticker-text", ".ticker-text", ".ticker-content", ".news-ticker"
    ].forEach(function (sel) {
      document.querySelectorAll(sel).forEach(function (el) { out.push(el); });
    });
    document.querySelectorAll("div,span,p,li,strong").forEach(function (el) {
      var t = (el.textContent || "").toLowerCase();
      if (t.indexOf("connecting to the scottish rock news") !== -1) out.push(el);
    });
  ...
  async function refreshTicker() {
    try {
      ...
      var text = (rows || []).map(function (r) { return r && r.title; }).filter(Boolean).join("   •   ");
      if (!text) text = "No Scottish Rock News available right now.";
      targets().forEach(function (el) { el.textContent = text; });
    } catch (e) {
      console.error("WIRE_MAIN_FALLBACK_V1", e);
    }
  }

  window.addEventListener("load", function () { setTimeout(refreshTicker, 1200); });
