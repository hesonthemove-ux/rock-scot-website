# Create fetch_wire_rss on the Pi (if the folder is missing from git)

Run these on the **Pi** in one go. This creates the function file so you can deploy.

```bash
cd ~/rock-scot-website
mkdir -p supabase/functions/fetch_wire_rss
```

Then create the file (one command; the backslash before each newline is required):

```bash
cat > supabase/functions/fetch_wire_rss/index.ts << 'ENDOFFILE'
/**
...
serve(async (req) => {
  ...
  try {
    ...
    for (const feed of FEEDS) {
      try {
        ...
        for (const item of items) {
          ...
          const link = (item.link ?? item.guid ?? 'rss-' + feed.sourceName + '-' + Date.now()).trim()
          if (!title) continue
    ...
    return new Response(
      JSON.stringify({ ok: true, inserted, errors: errors.length ? errors : undefined, message: 'Inserted ' + inserted + ' items.' }),
...
```bash
ls -la supabase/functions/fetch_wire_rss/
